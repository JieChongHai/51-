...
class GoodsLogic extends RelationModel
{
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        $list_brand = M('brand')->where("id in(select brand_id from goods where brand_id > 0 and goods_id in($goods_id_str))")->limit('30')->select();  //where("goods_id in($goods_id_str)")->select();
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $spec_key = M('spec_goods_price')->query("select group_concat(`key` separator  '_') as `key` from spec_goods_price where goods_id in($goods_id_str)");  //where("goods_id in($goods_id_str)")->select();
