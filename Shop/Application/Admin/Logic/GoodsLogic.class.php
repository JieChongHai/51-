...
class GoodsLogic extends RelationModel
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                $sql = "SELECT * FROM  goods_category ORDER BY parent_id , sort_order ASC";
     ...
     * @return 返回数组 Description
     */
    public function refresh_category($id)
    {            
        ...
        {
            ($cat['parent_id'] == 0) && $GoodsCategory->execute("UPDATE goods_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            $GoodsCategory->execute("UPDATE goods_category AS a ,goods_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $GoodsCategory->where("id = $id")->find(); // 从新找出他自己
        }        
        
        if($cat['parent_id'] == 0) //有可能是顶级分类，没有上级分类
        {
            $parent_cat['parent_id_path'] =   '0';   
            $parent_cat['level'] = 0;
        }
        else{
            $parent_cat = $GoodsCategory->where("id = {$cat['parent_id']}")->find(); // 找出上级分类的parent_id_path
        }        
        $replace_level = $cat['level'] - ($parent_cat['level'] + 1); // 查看比原来的等级上升了多少  ($parent_cat['level'] + 1) 上级等级加一 就是现在要改的等级
        $replace_str = $parent_cat['parent_id_path'].'_'.$id;                
        $GoodsCategory->execute("UPDATE `goods_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
    }    
