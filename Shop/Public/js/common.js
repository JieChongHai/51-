...
function AjaxAddCart(goods_id,num,to_catr)
{
	...
	if($("#buy_goods_form").length > 0){
		$.ajax({
			type : "POST",
			url:"/index.php?m=Home&c=Cart&a=ajaxAddCart",	// 请求地址
			data : $('#buy_goods_form').serialize(),		// 搜索表单 序列化提交
			dataType:'json',								// 数据格式
			success: function(data){						// 请求成功后的相应
					if(data.status < 0){
				   ...
				   // 加入购物车后再跳转到 购物车页面
				   if(to_catr == 1){  //直接购买
					   location.href = "/index.php?m=Home&c=Cart&a=cart"; // 跳转到购物车页面
				   }else{
						...
						layer.open({
							  ...
							  area: ['490px', '386px'], //宽高
							  content:["/index.php?m=Home&c=Goods&a=open_add_cart","no"],	// 弹出框页面内容
